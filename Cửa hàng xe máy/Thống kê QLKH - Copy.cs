...
{
    public partial class Thống_kê_QLSP : Form
    {
        SqlConnection con;
        SqlCommand cmd;
        SqlDataReader dr;
        ...
        private void cbbMaLoai_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            da = new SqlDataAdapter("Select * from QLKH where MaLoaiSP='" + ma.Trim() + "'", con);
            dt = new DataTable();
            da.Fill(dt);
            dgvThongKe.DataSource = dt;
        }

        private void Thống_kê_QLSP_Load(object sender, EventArgs e)
        {
            loadcbb();
        }
